...
func _run() -> void:
	if not FileAccess.file_exists(WORLD_PATH):
		return _fail("L1 world missing; run python tools/scale_ladder.py --level L1 --force")
	...
	_scene_root.world_max = Vector3(255.999, 63.999, 255.999)
	...
	if not await _wait_for_world(terrain):
		return _fail("L1 world did not become ready")
	...
	if not await _wait_for_settled_resources(terrain):
		return _fail("L1 center streaming did not settle")
	var settle_ms := Time.get_ticks_msec() - settle_started
	if not await _audit_density_signs(terrain):
		return

	var positions: Array[Vector3] = [
		Vector3(32.0, 56.0, 32.0),
		Vector3(64.0, 56.0, 128.0),
		Vector3(128.0, 56.0, 128.0),
		Vector3(192.0, 56.0, 128.0),
		Vector3(224.0, 56.0, 224.0),
	]
	...
	for step in range(positions.size()):
		...
		viewer.emit_signal("position_changed", viewer.global_position)
		if not await _wait_for_settled_resources(terrain):
			return _fail("L1 staged movement did not settle at step " + str(step))
		...
		for offset in offsets:
			...
			if not _probe_render_and_collision(terrain, probe):
				return _fail("L1 render/collision probe failed at " + str(probe))
	...
	if not terrain.stop_world():
		return _fail("L1 world stop was rejected")
	for _frame in range(TIMEOUT_FRAMES):
		if terrain.get_world_state_name() == "stopped":
			...
			return
		await process_frame
	_fail("L1 world did not stop after runtime audit")

func _audit_density_signs(terrain: Node) -> bool:
	var below := await _query_sample(terrain, Vector3i(128, 20, 128))
	var above := await _query_sample(terrain, Vector3i(128, 60, 128))
	var boundary := await _query_sample(terrain, Vector3i(0, 20, 128))
	var inner := await _query_sample(terrain, Vector3i(2, 20, 128))
	if below == null or above == null or boundary == null or inner == null:
		_fail("L1 density sign queries failed")
		return false
	if below.get_density() >= 0.0 or above.get_density() <= 0.0:
		_fail("L1 center density signs are inconsistent")
		return false
	if boundary.get_density() <= 0.0 or inner.get_density() >= 0.0:
		_fail("L1 finite boundary density is inconsistent")
		return false
	return true

func _audit_edit_latency(terrain: Node) -> float:
	var point := Vector3i(128, 28, 128)
	var before := await _query_sample(terrain, point)
	if before == null:
		_fail("L1 pre-edit sample failed")
		return NAN
	...
	if transaction == null or not transaction.add_density_sphere(
		...
		_fail("L1 edit submission failed")
		return NAN
	for _frame in range(TIMEOUT_FRAMES):
		var metrics: Dictionary = terrain.get_runtime_metrics()
		if terrain.get_world_revision() > revision_before and \
			...
			if after == null:
				_fail("L1 post-edit sample failed")
				return NAN
			var delta: float = after.get_density() - before.get_density()
			if absf(delta - 6.0) > 0.001:
				_fail("L1 edit density delta mismatch: " + str(delta))
				return NAN
			return delta
		await process_frame
	_fail("L1 edit did not commit and settle")
	return NAN
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L1_RUNTIME_FAIL: " + message)
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
