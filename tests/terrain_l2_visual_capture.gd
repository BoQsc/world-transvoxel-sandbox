...
const OUTPUT_ROOT := "res://artifacts/scale_ladder/L2/visual"
...
func _run() -> void:
	...
	_scene_root.get_node("Visualizer").world_bounds_extent = Vector3(512, 64, 512)
	...
	if not await _wait_for_world(terrain):
		return _fail("L2 world did not start for visual capture")

	if not await _view(viewer, terrain, Vector3(256, 76, 444), Vector3(256, 30, 256)):
		return
	...
	if not await _view(viewer, terrain, Vector3(256, 96, 256), Vector3(256, 24, 256), Vector3.FORWARD):
		return
	...
	if not await _view(viewer, terrain, Vector3(240, 17, 343), Vector3(272, 14, 354)):
		return
	...
	if not await _view(viewer, terrain, Vector3(-24, 38, 256), Vector3(18, 28, 256)):
		return
	...
	if not await _capture("closed_boundary_material"):
		return
	print("WT_SANDBOX_L2_VISUAL_CAPTURE_PASS images=7 active_capacity=%d" % ACTIVE_CHUNK_CAPACITY)
...
func _capture(name: String) -> bool:
	...
	if image.save_png(output) != OK:
		...
		return false
	print(
		"WT_SANDBOX_L2_CAPTURE image=%s size=%dx%d range=%.3f"
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L2_VISUAL_CAPTURE_FAIL: " + message)
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
