extends SceneTree
const WORLD_ROOT := "res://artifacts/scale_ladder/L2/world"
...
func _run() -> void:
	if not FileAccess.file_exists(WORLD_PATH):
		return _fail("L2 world missing; run python tools/scale_ladder.py --level L2 --force")
	...
	_scene_root.world_max = Vector3(511.999, 63.999, 511.999)
	...
	viewer.position = Vector3(64.0, 56.0, 64.0)
	...
	if not await _wait_for_world(terrain):
		return _fail("L2 world did not become ready")
	...
	if not await _wait_for_settled_resources(terrain):
		return _fail("L2 center streaming did not settle")
	...
	var positions: Array[Vector3] = [
		...
		Vector3(192.0, 56.0, 192.0),
		Vector3(256.0, 56.0, 256.0),
		Vector3(320.0, 56.0, 320.0),
	]
	...
	for step in range(positions.size()):
		...
		viewer.emit_signal("position_changed", viewer.global_position)
		if not await _wait_for_settled_resources(terrain):
			return _fail("L2 staged movement did not settle at step " + str(step))
		...
		for offset in offsets:
			...
			if not _probe_render_and_collision(terrain, probe):
				return _fail("L2 render/collision probe failed at " + str(probe))
	...
	if not terrain.stop_world():
		return _fail("L2 world stop was rejected")
	for _frame in range(TIMEOUT_FRAMES):
		if terrain.get_world_state_name() == "stopped":
			...
			return
		await process_frame
	_fail("L2 world did not stop after runtime audit")
func _audit_density_signs(terrain: Node) -> bool:
	var below := await _query_sample(terrain, Vector3i(256, 20, 256))
	var above := await _query_sample(terrain, Vector3i(256, 60, 256))
	var boundary := await _query_sample(terrain, Vector3i(0, 20, 256))
	var inner := await _query_sample(terrain, Vector3i(2, 20, 256))
	if below == null or above == null or boundary == null or inner == null:
		_fail("L2 density sign queries failed")
		return false
	if below.get_density() >= 0.0 or above.get_density() <= 0.0:
		_fail("L2 center density signs are inconsistent")
		return false
	if boundary.get_density() <= 0.0 or inner.get_density() >= 0.0:
		_fail("L2 finite boundary density is inconsistent")
		return false
	return true
func _audit_edit_latency(terrain: Node) -> float:
	var point := Vector3i(256, 28, 256)
	var before := await _query_sample(terrain, point)
	if before == null:
		_fail("L2 pre-edit sample failed")
		return NAN
	var revision_before: int = terrain.get_world_revision()
	var mesh_jobs_before := int(terrain.get_runtime_metrics().get("mesh_jobs", 0))
	var transaction: Object = terrain.begin_edit_transaction(201)
	if transaction == null or not transaction.add_density_sphere(
		...
		_fail("L2 edit submission failed")
		return NAN
	for _frame in range(TIMEOUT_FRAMES):
		var metrics: Dictionary = terrain.get_runtime_metrics()
		if terrain.get_world_revision() > revision_before and \
			...
			if after == null:
				_fail("L2 post-edit sample failed")
				return NAN
			var delta: float = after.get_density() - before.get_density()
			if absf(delta - 6.0) > 0.001:
				_fail("L2 edit density delta mismatch: " + str(delta))
				return NAN
			return delta
		await process_frame
	_fail("L2 edit did not commit and settle")
	return NAN
...
func _wait_for_world(terrain: Node) -> bool:
	for _frame in range(TIMEOUT_FRAMES):
		...
		if terrain.get_world_state_name() == "failed":
			print("WT_SANDBOX_L2_RUNTIME_METRICS " + str(terrain.get_runtime_metrics()))
			return false
		await process_frame
	return false
func _wait_for_settled_resources(terrain: Node) -> bool:
	...
	for _frame in range(TIMEOUT_FRAMES):
		var metrics: Dictionary = terrain.get_runtime_metrics()
		if terrain.get_world_state_name() == "failed":
			print("WT_SANDBOX_L2_RUNTIME_METRICS " + str(metrics))
			return false
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L2_RUNTIME_FAIL: " + message)
	if _terrain != null:
		print("WT_SANDBOX_L2_RUNTIME_METRICS " + str(_terrain.get_runtime_metrics()))
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
