...
func _run() -> void:
	...
	if not await _wait_for_world(terrain):
		return _fail("L3 world did not become ready")
	...
	if not await _wait_for_settled_resources(terrain):
		return _fail("L3 center streaming did not settle")
	...
	var positions: Array[Vector3] = [
		...
		Vector3(384.0, 56.0, 384.0),
		Vector3(512.0, 56.0, 512.0),
		Vector3(640.0, 56.0, 640.0),
		Vector3(768.0, 56.0, 768.0),
		Vector3(896.0, 56.0, 896.0),
	]
	...
	for step in range(positions.size()):
		...
		viewer.emit_signal("position_changed", viewer.global_position)
		if not await _wait_for_settled_resources(terrain):
			return _fail("L3 staged movement did not settle at step " + str(step))
		...
		for offset in offsets:
			...
			if not _probe_render_and_collision(terrain, probe):
				return _fail("L3 render/collision probe failed at " + str(probe))
	...
	if not terrain.stop_world():
		return _fail("L3 world stop was rejected")
	for _frame in range(TIMEOUT_FRAMES):
		if terrain.get_world_state_name() == "stopped":
			print(
				"WT_SANDBOX_L3_RUNTIME_PASS startup_ms=%d settle_ms=%d positions=%d probes=%d min_render=%d min_collision=%d edit_ms=%d edit_delta=%.1f max_retiring=%d active_capacity=%d render_apply_budget=%d"
				% [
					startup_ms,
					settle_ms,
					positions.size(),
			...
			return
		await process_frame
	_fail("L3 world did not stop after runtime audit")
...
func _audit_edit_latency(terrain: Node) -> float:
	...
	if transaction == null or not transaction.add_density_sphere(
		...
		_fail("L3 edit submission failed")
		return NAN
	for _frame in range(TIMEOUT_FRAMES):
		var metrics: Dictionary = terrain.get_runtime_metrics()
		if terrain.get_world_revision() > revision_before and \
			...
			if after == null:
				_fail("L3 post-edit sample failed")
				return NAN
			var delta: float = after.get_density() - before.get_density()
			if absf(delta - 6.0) > 0.001:
				_fail("L3 edit density delta mismatch: " + str(delta))
				return NAN
			return delta
		await process_frame
	_fail("L3 edit did not commit and settle")
	return NAN
...
func _wait_for_world(terrain: Node) -> bool:
	for _frame in range(TIMEOUT_FRAMES):
		...
		if terrain.get_world_state_name() == "failed":
			print("WT_SANDBOX_L3_RUNTIME_METRICS " + str(terrain.get_runtime_metrics()))
			return false
		await process_frame
	return false
func _wait_for_settled_resources(terrain: Node) -> bool:
	...
	for _frame in range(TIMEOUT_FRAMES):
		var metrics: Dictionary = terrain.get_runtime_metrics()
		if terrain.get_world_state_name() == "failed":
			print("WT_SANDBOX_L3_RUNTIME_METRICS " + str(metrics))
			return false
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L3_RUNTIME_FAIL: " + message)
	if _terrain != null: print("WT_SANDBOX_L3_RUNTIME_METRICS " + str(_terrain.get_runtime_metrics()))
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
