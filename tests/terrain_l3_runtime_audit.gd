...
func _run() -> void:
	...
	for _frame in range(TIMEOUT_FRAMES):
		if terrain.get_world_state_name() == "stopped":
			print(
				"WT_SANDBOX_L3_RUNTIME_PASS startup_ms=%d settle_ms=%d positions=%d probes=%d min_render=%d min_collision=%d edit_ms=%d edit_delta=%.1f max_retiring=%d active_capacity=%d"
				% [
					startup_ms,
					settle_ms,
					positions.size(),
...
func _fail(message: String) -> void:
	...
	if _terrain != null:
		print("WT_SANDBOX_L3_RUNTIME_METRICS " + str(_terrain.get_runtime_metrics()))
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
