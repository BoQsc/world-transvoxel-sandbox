...
const OUTPUT_ROOT := "res://artifacts/scale_ladder/L4/visual"
...
func _run() -> void:
	...
	_scene_root.get_node("Visualizer").world_bounds_extent = Vector3(2048, 64, 2048)
	...
	if not await _wait_for_world(terrain):
		return _fail("L4 world did not start for visual capture")

	if not await _view(viewer, terrain, Vector3(1024, 76, 1776), Vector3(1024, 30, 1024)):
		return
	...
	if not await _view(viewer, terrain, Vector3(1024, 96, 1024), Vector3(1024, 24, 1024), Vector3.FORWARD):
		return
	...
	if not await _view(viewer, terrain, Vector3(992, 17, 1395), Vector3(1024, 13, 1392)):
		return
	...
	sun.shadow_enabled = false
	await process_frame
	if not await _view(viewer, terrain, Vector3(-24, 38, 1024), Vector3(18, 28, 1024)):
		return
	var boundary_query := PhysicsRayQueryParameters3D.create(
		Vector3(-24, 12, 1024), Vector3(12, 12, 1024)
	)
	...
	if boundary_hit.is_empty() or absf(float(boundary_hit["position"].x) - 0.5) > 0.01:
		return _fail("L4 targeted finite boundary collision ray failed: " + str(boundary_hit))
	print("WT_SANDBOX_L4_BOUNDARY_PASS hit_x=%.3f" % float(boundary_hit["position"].x))
	...
	if not await _capture("closed_boundary_material"):
		return
	print("WT_SANDBOX_L4_VISUAL_CAPTURE_PASS images=7 active_capacity=%d" % ACTIVE_CHUNK_CAPACITY)
...
func _capture(name: String) -> bool:
	...
	if image.save_png(output) != OK:
		...
		return false
	print(
		"WT_SANDBOX_L4_CAPTURE image=%s size=%dx%d range=%.3f"
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L4_VISUAL_CAPTURE_FAIL: " + message)
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
