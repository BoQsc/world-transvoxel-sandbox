...
const OUTPUT_ROOT := "res://artifacts/scale_ladder/L1/visual"
...
func _run() -> void:
	...
	_scene_root.get_node("Visualizer").world_bounds_extent = Vector3(256, 64, 256)
	...
	if not await _wait_for_world(terrain):
		return _fail("L1 world did not start for visual capture")

	if not await _view(viewer, terrain, Vector3(128, 76, 222), Vector3(128, 30, 128)):
		return
	...
	if not await _view(viewer, terrain, Vector3(128, 96, 128), Vector3(128, 24, 128), Vector3.FORWARD):
		return
	...
	if not await _view(viewer, terrain, Vector3(128, 13, 174), Vector3(144, 16, 170)):
		return
	...
	if not await _view(viewer, terrain, Vector3(-24, 38, 128), Vector3(18, 28, 128)):
		return
	...
	if not await _capture("closed_boundary_material"):
		return
	print("WT_SANDBOX_L1_VISUAL_CAPTURE_PASS images=7")
...
func _capture(name: String) -> bool:
	...
	if image.save_png(output) != OK:
		...
		return false
	print(
		"WT_SANDBOX_L1_CAPTURE image=%s size=%dx%d range=%.3f"
...
func _fail(message: String) -> void:
	push_error("WT_SANDBOX_L1_VISUAL_CAPTURE_FAIL: " + message)
	if _scene_root != null:
		_scene_root.queue_free()
	quit(1)
